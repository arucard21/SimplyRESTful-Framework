...
public class HALCollectionBuilderFromFullListTest extends AbstractHALCollectionBuilderTest {
	...
	public void test_createPagedCollectionFromFullList_WithPrevPage() {
		...
		boolean compact = true;
		HALCollectionV1<TestResource> actual = builder.page(page).maxPageSize(maxPageSize).compact(compact).build();
		HALCollectionV1<TestResource> expected = createExpectedCollection(page, maxPageSize, 1, 10, 2, 4, 200, 300, compact);
		Assertions.assertEquals(expected, actual);
	}
	...
	public void test_createPagedCollectionFromFullList_WithoutNextPage() {
		...
		boolean compact = true;
		HALCollectionV1<TestResource> actual = builder.page(page).maxPageSize(maxPageSize).compact(compact).build();
		HALCollectionV1<TestResource> expected = createExpectedCollection(page, maxPageSize, 1, 10, 9, -1, 900, 1000, compact);
		Assertions.assertEquals(expected, actual);
	}
