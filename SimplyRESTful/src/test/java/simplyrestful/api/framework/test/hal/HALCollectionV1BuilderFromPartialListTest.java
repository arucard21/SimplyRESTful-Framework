...
public class HALCollectionV1BuilderFromPartialListTest extends AbstractHALCollectionV1BuilderTest {
    @Test
    public void test_createPagedCollectionFromPartialList_DefaultValues() {
	int page = 1;
	int maxPageSize = 100;
	boolean compact = true;
	builder = HALCollectionV1Builder.fromPartial(testResourcesList.subList(0, 100), requestURI,
		TEST_RESOURCES_SIZE);
	...
	int maxPageSize = 100;
	boolean compact = true;
	builder = HALCollectionV1Builder.fromPartial(testResourcesList.subList(900, 1000), requestURI,
		TEST_RESOURCES_SIZE);
	HALCollectionV1<TestResource> actual = builder.page(page).maxPageSize(maxPageSize).compact(compact).build();
